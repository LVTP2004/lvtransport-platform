...
Tags: bootstrap,convergence,implementation,map,moni,founder

## Purpose

This document maps current implementation fragments that must converge before building new platform layers.
...
- Canonical shared contracts now exist in packages/shared.
- API already contains the closest canonical RideStatus in apps/api/src/persistence/repository-contracts.ts.
- Tracking generation remains fragmented across API, booking, notification and Moni assistant flows.
- HomeOriginal still contains operational localStorage booking state.
- Dispatch is still coupled to dispatchMvpStore and must be separated into a DispatchAssignment contract.

## Decision

This convergence map must be reviewed before creating new Builder, Planner, Finance or Organization layers.
