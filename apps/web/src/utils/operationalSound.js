...
export function isOperationalSoundEnabled() {
    return readPreference();
}
export function setOperationalSoundEnabled(enabled) {
    if (typeof window === 'undefined')
        return;
    window.localStorage.setItem(STORAGE_KEY, String(enabled));
}
export function toggleOperationalSound() {
    const next = !isOperationalSoundEnabled();
    setOperationalSoundEnabled(next);
    return next;
}
export async function playOperationalSound(event) {
    if (typeof window === 'undefined')
        return;
    if (!isOperationalSoundEnabled())
        return;
...
export function getOperationalSoundPath(event) {
