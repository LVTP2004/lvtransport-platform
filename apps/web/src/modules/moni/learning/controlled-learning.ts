import type { MoniIntent, MoniLanguage, MoniLearningClassification, MoniLearningRecord } from '../types/moni.types';

const toScore = (value: number): number => Math.max(0, Math.min(100, Math.round(value)));

const classifyInteraction = (intent: MoniIntent, text: string, reply: string): MoniLearningClassification => {
  if (/complaint|klacht|problem|issue/i.test(text)) return 'complaint';
  if (/vip|business|zakelijk|airport|luchthaven/i.test(text)) return 'airport_vip_request';
  if (/price|prijs|cost|tarief/i.test(text)) return 'price_question';
  if (/track|status|volg|where is/i.test(text)) return 'tracking_issue';
  if (/help|unclear|confus|wat bedoel/i.test(text)) return 'customer_confusion';
  if (intent === 'booking_request' && /missing|details/i.test(reply)) return 'incomplete_booking';
  if (intent === 'escalation_request') return 'escalation_needed';
  if (intent === 'general_question') return 'potential_improvement';
  return intent === 'booking_request' ? 'successful_booking' : 'unclear_intent';
};

const detectWeaknesses = (text: string, classification: MoniLearningClassification): string[] => {
  const patterns: string[] = [];
  if (/\?{2,}|what|hoe|wat/i.test(text)) patterns.push('confusing_questions');
  if (/price|prijs/i.test(text)) patterns.push('unclear_price_explanations');
  if (/track|volg|status/i.test(text)) patterns.push('tracking_confusion');
  if (/airport|luchthaven|vip|business/i.test(text)) patterns.push('premium_flow_expectation_gap');
  if (classification === 'incomplete_booking') patterns.push('repeated_missing_information');
  if (!patterns.length) patterns.push('none_detected');
  return patterns;
};

const suggestImprovement = (classification: MoniLearningClassification): string => {
...
export const createLearningRecord = (input: {
  ...
  const operationalRisk: 'low' | 'medium' | 'high' =
    classification === 'complaint' || classification === 'escalation_needed' ? 'high' :
...
export const persistLearningRecord = (record: MoniLearningRecord): void => {
  const storageKey = 'moni_controlled_learning_records';
  const current = globalThis.localStorage?.getItem(storageKey);
  const parsed: MoniLearningRecord[] = current ? JSON.parse(current) : [];
