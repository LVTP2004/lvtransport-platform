...
export function MoniAssistant() {
    ...
    const send = (text) => {
        ...
        if (intent === 'tracking_request')
            replies.push('Gebruik uw reserveringscode op /tracking/{code}.');
        if (intent === 'price_request')
            replies.push('Ik geef enkel een estimated price. Definitieve prijs volgt via het boekingsformulier.');
