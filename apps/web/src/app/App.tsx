...
export function App() {
  ...
  return <div className='min-h-screen bg-lv-black px-3 py-4 text-white sm:px-6 lg:px-8'><div className='mx-auto w-full max-w-6xl'><section className='mb-4 grid gap-2 md:grid-cols-2'><div className='rounded-2xl border border-white/10 bg-black/30 px-3 py-2 text-xs text-lv-mist'>Connectivity: <span className={isOnline ? 'text-emerald-300' : 'text-amber-300'}>{isOnline ? 'Online realtime' : 'Offline-safe mode'}</span></div><div className='rounded-2xl border border-white/10 bg-black/30 px-3 py-2 text-xs text-lv-mist'>PWA install: <span className='text-lv-champagne'>{installReady ? 'Ready' : 'Pending browser prompt'}</span></div></section><section className='grid gap-4 lg:grid-cols-[1.2fr_0.8fr]'><div className='glass-panel rounded-3xl p-4 sm:p-6'>{confirmation ? <div className='space-y-4'><p className='text-xs uppercase tracking-[0.2em] text-lv-champagne'>Booking confirmed</p><h2 className='text-2xl font-semibold'>Reference: {confirmation.referenceCode}</h2><p className='text-lv-mist'>Status: {confirmation.status}</p><p className='text-xs text-lv-mist'>Keep this booking code for support and status checks.</p><Button className='shadow-gold-md' onClick={resetDraft}>Create another booking</Button></div> : <><div className='mb-4 flex items-center justify-between'><p className='text-sm text-lv-mist'>Step {step} of 3</p><p className='text-xs text-lv-mist'>All fields are required for dispatch readiness.</p></div>{step === 1 && <div className='space-y-4'><label className='field-wrap'><span>Pickup</span><input value={pickup} onChange={(e) => setPickup(e.target.value)} /></label><label className='field-wrap'><span>Destination</span><input value={destination} onChange={(e) => setDestination(e.target.value)} /></label><label className='field-wrap'><span>Date & time</span><input type='datetime-local' value={dateTime} onChange={(e) => setDateTime(e.target.value)} /></label></div>}{step === 2 && <div className='space-y-4'><div className='field-wrap'><span>Passengers</span><div className='mt-2 flex items-center justify-between rounded-2xl border border-lv-gold/20 bg-white/5 px-4 py-3'><button className='control-btn' onClick={() => setPassengers((v) => Math.max(1, v - 1))}>−</button><strong className='text-lg'>{passengers}</strong><button className='control-btn' onClick={() => setPassengers((v) => Math.min(12, v + 1))}>+</button></div></div><div className='grid gap-2'>{vehicles.map((item) => <button key={item.name} onClick={() => setVehicle(item)} className={`vehicle-card ${vehicle.name === item.name ? 'vehicle-card--active' : ''}`}><div><p className='font-medium'>{item.name}</p><p className='text-xs text-lv-mist'>ETA {item.eta} • up to {item.seats} passengers</p></div></button>)}</div></div>}{step === 3 && <div className='space-y-3'><button className={`toggle-card ${airportTransfer ? 'toggle-card--active' : ''}`} onClick={() => setAirportTransfer((v) => !v)}>Airport transfer</button><button className={`toggle-card ${businessVip ? 'toggle-card--active' : ''}`} onClick={() => setBusinessVip((v) => !v)}>Business / VIP</button></div>}{info && <p className='mt-3 text-sm text-lv-champagne'>{info}</p>}{error && <p className='mt-3 text-sm text-rose-300'>{error}</p>}<div className='mt-5 flex gap-2'><Button variant='secondary' className='flex-1' disabled={loading} onClick={() => setStep((v) => (v > 1 ? ((v - 1) as Step) : v))}>Back</Button>{step < 3 ? <Button className='flex-1' disabled={loading} onClick={() => setStep((v) => (v < 3 ? ((v + 1) as Step) : v))}>Continue</Button> : <Button className='flex-1 shadow-gold-md' onClick={submitBooking} disabled={loading || !pickup || !destination || !dateTime || !isOnline}>{loading ? 'Submitting...' : 'Confirm booking'}</Button>}</div></>}</div><aside className='space-y-4'><article className='glass-panel rounded-3xl p-4'><p className='text-xs uppercase tracking-[0.2em] text-lv-champagne'>Price estimate</p><p className='mt-2 text-3xl font-semibold'>${baseFare}</p></article><article className='glass-panel rounded-3xl p-4'><p className='text-xs uppercase tracking-[0.2em] text-lv-champagne'>Booking summary</p><ul className='mt-3 space-y-2 text-sm'><li>Pickup: {pickup || 'Not set'}</li><li>Destination: {destination || 'Not set'}</li><li>Schedule: {formatDateTime(dateTime)}</li></ul></article></aside></section></div><MoniAssistant /></div>;
}
