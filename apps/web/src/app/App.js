...
export function App() {
    ...
    const Header = _jsx("header", { className: 'glass-panel rounded-3xl p-5', children: _jsx("nav", { className: 'flex flex-wrap gap-2 text-sm', children: [['/', 'Book'], ['/prijzen', 'Prices'], ['/tracking', 'Track'], ['/diensten', 'Services'], ['/contact', 'Contact'], ['/driver', 'Driver'], ['/admin', 'Admin']].map(([p, l]) => _jsx("button", { onClick: () => navigate(p), className: 'rounded-lg border border-lv-gold/30 px-3 py-1.5', children: l }, p)) }) });
    if (route === '404')
