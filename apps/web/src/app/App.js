...
const formatDateTime = (value) => !value ? 'Nog niet gekozen' : new Date(value).toLocaleString('nl-BE', { weekday: 'short', month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' });
...
export function App() {
    ...
    const submitBooking = async () => {
        ...
        setError('');
        setInfo('Je boeking wordt veilig verwerkt...');
        setLoading(true);
        ...
        try {
            ...
            if (!response.ok)
                throw new Error(payload?.message ?? 'Boeking kon niet worden aangemaakt.');
    ...
    return _jsxs("div", { className: 'min-h-screen bg-lv-black px-3 py-4 text-white sm:px-6 lg:px-8', children: [_jsxs("div", { className: 'mx-auto w-full max-w-6xl', children: [_jsxs("header", { className: 'mb-4 rounded-3xl border border-lv-gold/30 bg-black/40 p-4', children: [_jsx("p", { className: 'text-xs uppercase tracking-[0.2em] text-lv-champagne', children: "LV Transport \u00B7 Antwerpen" }), _jsx("h1", { className: 'mt-2 text-2xl font-semibold', children: "Premium rit boeken" }), _jsx("p", { className: 'mt-2 text-sm text-lv-mist', children: "Dutch first \u00B7 EN/ES support on request. Zakelijke en luchthavenritten met realtime opvolging." })] }), _jsxs("section", { className: 'mb-4 grid gap-2 md:grid-cols-2', children: [_jsxs("div", { className: 'rounded-2xl border border-white/10 bg-black/30 px-3 py-2 text-xs text-lv-mist', children: ["Verbinding: ", _jsx("span", { className: isOnline ? 'text-emerald-300' : 'text-amber-300', children: isOnline ? 'Online · live tracking actief' : 'Offline · veilig opgeslagen, later verzenden' })] }), _jsxs("div", { className: 'rounded-2xl border border-white/10 bg-black/30 px-3 py-2 text-xs text-lv-mist', children: ["App install: ", _jsx("span", { className: 'text-lv-champagne', children: installReady ? 'Beschikbaar' : 'Nog niet aangeboden door browser' })] })] }), _jsxs("section", { className: 'grid gap-4 lg:grid-cols-[1.2fr_0.8fr]', children: [_jsx("div", { className: 'glass-panel rounded-3xl p-4 sm:p-6', children: confirmation ? _jsxs("div", { className: 'space-y-4', children: [_jsx("p", { className: 'text-xs uppercase tracking-[0.2em] text-lv-champagne', children: "Boeking bevestigd" }), _jsxs("h2", { className: 'text-2xl font-semibold', children: ["Referentie: ", confirmation.referenceCode] }), _jsxs("p", { className: 'text-lv-mist', children: ["Status: ", statusLabel(confirmation.status)] }), _jsx("p", { className: 'text-sm text-lv-mist', children: "Wat nu? 1) Chauffeur wordt toegewezen 2) Je ontvangt tracking 3) Je ziet wanneer de chauffeur onderweg en aangekomen is." }), _jsx("p", { className: 'text-xs text-lv-mist', children: "Contact nodig? Bel LV Transport met je referentiecode voor directe opvolging." }), _jsx(Button, { className: 'shadow-gold-md', onClick: resetDraft, children: "Nieuwe boeking starten" })] }) : _jsxs(_Fragment, { children: [_jsxs("div", { className: 'mb-4 flex items-center justify-between', children: [_jsxs("p", { className: 'text-sm text-lv-mist', children: ["Stap ", step, " van 3"] }), _jsx("p", { className: 'text-xs text-lv-mist', children: "Heldere gegevens zorgen voor snelle dispatch." })] }), step === 1 && _jsxs("div", { className: 'space-y-4', children: [_jsxs("label", { className: 'field-wrap', children: [_jsx("span", { children: "Ophaallocatie" }), _jsx("input", { placeholder: 'Bijv. Antwerpen-Centrum', value: pickup, onChange: (e) => setPickup(e.target.value) })] }), _jsxs("label", { className: 'field-wrap', children: [_jsx("span", { children: "Bestemming" }), _jsx("input", { placeholder: 'Bijv. Brussels Airport', value: destination, onChange: (e) => setDestination(e.target.value) })] }), _jsxs("label", { className: 'field-wrap', children: [_jsx("span", { children: "Datum & uur" }), _jsx("input", { type: 'datetime-local', value: dateTime, onChange: (e) => setDateTime(e.target.value) })] })] }), step === 2 && _jsxs("div", { className: 'space-y-4', children: [_jsxs("div", { className: 'field-wrap', children: [_jsx("span", { children: "Passagiers" }), _jsxs("div", { className: 'mt-2 flex items-center justify-between rounded-2xl border border-lv-gold/20 bg-white/5 px-4 py-3', children: [_jsx("button", { className: 'control-btn', onClick: () => setPassengers((v) => Math.max(1, v - 1)), children: "\u2212" }), _jsx("strong", { className: 'text-lg', children: passengers }), _jsx("button", { className: 'control-btn', onClick: () => setPassengers((v) => Math.min(12, v + 1)), children: "+" })] })] }), _jsx("div", { className: 'grid gap-2', children: vehicles.map((item) => _jsx("button", { onClick: () => setVehicle(item), className: `vehicle-card ${vehicle.name === item.name ? 'vehicle-card--active' : ''}`, children: _jsxs("div", { children: [_jsx("p", { className: 'font-medium', children: item.name }), _jsxs("p", { className: 'text-xs text-lv-mist', children: ["ETA ", item.eta, " \u00B7 tot ", item.seats, " passagiers"] }), _jsx("p", { className: 'text-xs text-lv-mist', children: item.description })] }) }, item.name)) })] }), step === 3 && _jsxs("div", { className: 'space-y-3', children: [_jsx("button", { className: `toggle-card ${airportTransfer ? 'toggle-card--active' : ''}`, onClick: () => setAirportTransfer((v) => !v), children: "Luchthavenservice" }), _jsx("button", { className: `toggle-card ${businessVip ? 'toggle-card--active' : ''}`, onClick: () => setBusinessVip((v) => !v), children: "Business / VIP service" }), _jsx("p", { className: 'text-xs text-lv-mist', children: "Na bevestiging ontvang je statusupdates: toegewezen, onderweg, aangekomen, rit bezig, voltooid." })] }), info && _jsx("p", { className: 'mt-3 text-sm text-lv-champagne', children: info }), error && _jsx("p", { className: 'mt-3 text-sm text-rose-300', children: error }), _jsxs("div", { className: 'mt-5 flex gap-2', children: [_jsx(Button, { variant: 'secondary', className: 'flex-1', disabled: loading, onClick: () => setStep((v) => (v > 1 ? (v - 1) : v)), children: "Terug" }), step < 3 ? _jsx(Button, { className: 'flex-1', disabled: loading, onClick: () => setStep((v) => (v < 3 ? (v + 1) : v)), children: "Verder" }) : _jsx(Button, { className: 'flex-1 shadow-gold-md', onClick: submitBooking, disabled: loading || !pickup || !destination || !dateTime || !isOnline, children: loading ? 'Bezig met bevestigen...' : 'Boeking bevestigen' })] })] }) }), _jsxs("aside", { className: 'space-y-4', children: [_jsxs("article", { className: 'glass-panel rounded-3xl p-4', children: [_jsx("p", { className: 'text-xs uppercase tracking-[0.2em] text-lv-champagne', children: "Richtprijs" }), _jsxs("p", { className: 'mt-2 text-3xl font-semibold', children: ["\u20AC", baseFare] }), _jsx("p", { className: 'text-xs text-lv-mist', children: "Indicatief tarief, bevestigd bij toewijzing." })] }), _jsxs("article", { className: 'glass-panel rounded-3xl p-4', children: [_jsx("p", { className: 'text-xs uppercase tracking-[0.2em] text-lv-champagne', children: "Boekingsoverzicht" }), _jsxs("ul", { className: 'mt-3 space-y-2 text-sm', children: [_jsxs("li", { children: ["Pickup: ", pickup || 'Nog niet ingevuld'] }), _jsxs("li", { children: ["Bestemming: ", destination || 'Nog niet ingevuld'] }), _jsxs("li", { children: ["Planning: ", formatDateTime(dateTime)] }), _jsxs("li", { children: ["Service: ", serviceType.toUpperCase()] })] })] })] })] })] }), _jsx(MoniAssistant, {})] });
}
