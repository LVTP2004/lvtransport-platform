...
class LongDistanceRule {
    apply(context) {
        if (context.input.estimatedDistanceKm <= PRICING_CONSTANTS.LONG_DISTANCE_THRESHOLD_KM)
            return;
...
export class CentralizedPricingEngineService {
    ...
    calculateBreakdown(input) {
        ...
        const context = { input, components, subtotal };
