...
type AdminBooking = { id: string; referenceCode: string; serviceType: string; status: string; scheduledAt: string; paymentStatus?: string; customerType?: 'vip'|'business'|'standard' };
...
export function App() {
  ...
  useEffect(() => {
    ...
    const poller = window.setInterval(() => {
      ...
      setSyncState((prev) => prev === 'degraded' ? 'recovering' : prev);
      load();
    }, 12000);
  ...
  const completedBookings = useMemo(() => bookings.filter((b) => ['completed', 'done'].includes(b.status.toLowerCase())), [bookings]);
  const vipBusinessBookings = useMemo(() => bookings.filter((b) => ['vip', 'business'].includes((b.customerType ?? '').toLowerCase())), [bookings]);
  const paymentReady = useMemo(() => bookings.filter((b) => ['paid', 'authorized', 'settled'].includes((b.paymentStatus ?? '').toLowerCase())).length, [bookings]);

  return <main className="min-h-screen bg-zinc-900 text-zinc-100"><div className="grid min-h-screen grid-cols-1 lg:grid-cols-[260px_1fr]"><aside className="border-r border-zinc-800 bg-black/90 p-6"><div className="mb-8"><p className="text-xs uppercase tracking-[0.28em] text-zinc-500">LV Transport</p><h1 className="mt-1 text-2xl font-bold text-amber-300">Control Tower</h1></div><nav className="space-y-2">{navItems.map(({ label, icon }, index) => <button key={label} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2 text-sm ${index === 0 ? 'bg-amber-400/20 text-amber-200' : 'text-zinc-300 hover:bg-zinc-800/80 hover:text-white'}`}><span className="w-4 text-center">{icon}</span>{label}</button>)}</nav></aside><div className="flex flex-col"><header className="flex items-center justify-between border-b border-zinc-800 bg-zinc-950/80 px-5 py-4"><div><p className="text-xs uppercase tracking-[0.2em] text-zinc-400">Operations Center</p><p className="text-lg font-medium text-white">Regional Dispatch & Service Health</p></div><p className="text-xs text-zinc-400">Sync: <span className={syncState === 'live' ? 'text-emerald-300' : syncState === 'recovering' ? 'text-amber-200' : 'text-rose-300'}>{syncState.toUpperCase()}</span> • cycle {refreshCycle}</p></header><div className="space-y-5 p-5"><section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><MetricCard title="Revenue Visibility" value={`$${(bookings.length * 78).toLocaleString()}`} trend={`${paymentReady} payment-cleared bookings`} tone="gold" /><MetricCard title="Active Dispatches" value={`${activeBookings.length}`} trend={`${driverLocations.filter((d) => d.status === 'on-trip').length} drivers on-trip`} tone="emerald" /><MetricCard title="Driver Utilization" value={`${Math.round((driverLocations.filter((d) => d.status !== 'break').length / driverLocations.length) * 100)}%`} trend="Multi-driver live readiness" tone="blue" /><MetricCard title="Operational Alerts" value={syncState === 'degraded' ? '1' : '0'} trend={syncState === 'degraded' ? 'Realtime feed degraded' : 'No critical lifecycle mismatches'} tone="rose" /></section>
<section className="grid gap-5 xl:grid-cols-3"><div className="space-y-5 xl:col-span-2"><Panel title="Realtime Operations Map" icon={<span>🛰</span>}><div className="grid gap-4 lg:grid-cols-[2fr_1fr]"><div className="relative min-h-[260px] rounded-xl border border-zinc-700 bg-zinc-900/90"><div className="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(251,191,36,0.12),transparent_55%)]" />{driverLocations.map((d) => <div key={d.driverId} className="absolute -translate-x-1/2 -translate-y-1/2" style={{ top: `${latToTop(d.lat)}%`, left: `${lngToLeft(d.lng)}%` }}><div className={`h-3 w-3 rounded-full ${d.status === 'on-trip' ? 'bg-emerald-400' : d.status === 'available' ? 'bg-sky-400' : 'bg-amber-300'}`} /><p className="mt-1 whitespace-nowrap text-[10px] text-zinc-300">{d.vehicleCode}</p></div>)}</div><div className="space-y-2 text-xs">{driverLocations.map((driver) => <div key={driver.driverId} className="rounded-lg border border-zinc-800 bg-zinc-900 p-2"><p className="font-medium text-zinc-100">{driver.name} <span className="text-zinc-400">({driver.vehicleCode})</span></p><p className={statusTone(driver.status)}>{driver.status.toUpperCase()} • {driver.speedKph} km/h</p><p className="text-zinc-400">{driver.activeBookingRef ? `Booking ${driver.activeBookingRef}` : 'No active booking'}</p></div>)}</div></div><div className="mt-3 grid gap-2 text-xs text-zinc-300 sm:grid-cols-4"><p className="rounded-lg bg-zinc-900 p-2">Live drivers: <span className="text-white">{driverLocations.length}</span></p><p className="rounded-lg bg-zinc-900 p-2">Active bookings: <span className="text-white">{activeBookings.length}</span></p><p className="rounded-lg bg-zinc-900 p-2">Completed rides: <span className="text-white">{completedBookings.length}</span></p><p className="rounded-lg bg-zinc-900 p-2">Last sync: <span className="text-emerald-300">{lastSyncIso ? new Date(lastSyncIso).toLocaleTimeString() : 'waiting'}</span></p></div></Panel>
<Panel title="Booking Lifecycle Command" icon={<span>◈</span>}><div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left text-sm"><thead className="text-xs uppercase tracking-[0.16em] text-zinc-400"><tr>{['Reference','Service','Status','Payment','Schedule'].map((h) => <th key={h} className="px-2 py-2">{h}</th>)}</tr></thead><tbody>{bookings.map((row) => <tr key={row.id} className="border-t border-zinc-800 text-zinc-200"><td className="px-2 py-3">{row.referenceCode}</td><td className="px-2 py-3">{row.serviceType}</td><td className="px-2 py-3">{row.status}</td><td className="px-2 py-3">{row.paymentStatus ?? 'pending'}</td><td className="px-2 py-3">{new Date(row.scheduledAt).toLocaleString()}</td></tr>)}</tbody></table></div></Panel></div>
<div className="space-y-5"><Panel title="Dispatch Monitoring" icon={<span>⌖</span>}><div className="space-y-2 text-sm text-zinc-300"><p className="rounded-lg bg-zinc-900 p-2">Open dispatch queue: <span className="text-white">{Math.max(activeBookings.length - 1, 0)}</span></p><p className="rounded-lg bg-zinc-900 p-2">Driver state updates: <span className="text-white">{driverLocations.length * 12}/min</span></p><p className="rounded-lg bg-zinc-900 p-2">Reconnect resilience: <span className="text-emerald-300">Lifecycle sync retained</span></p></div></Panel><Panel title="Business / VIP Operations" icon={<span>✦</span>}><div className="space-y-2 text-sm text-zinc-300"><p className="rounded-lg border border-zinc-800 bg-zinc-900 p-2">VIP + Business bookings: <span className="text-white">{vipBusinessBookings.length}</span></p><p className="rounded-lg border border-zinc-800 bg-zinc-900 p-2">Payment-cleared rides: <span className="text-white">{paymentReady}</span></p><p className="rounded-lg border border-zinc-800 bg-zinc-900 p-2">Historical completed visibility: <span className="text-emerald-300">Enabled</span></p></div></Panel><Panel title="Operational Diagnostics" icon={<span>⚠</span>}><ul className="space-y-2 text-sm text-zinc-300"><li className={`rounded-lg p-2 ${syncState === 'degraded' ? 'border border-rose-500/30 bg-rose-500/10' : 'border border-emerald-500/30 bg-emerald-500/10'}`}>Realtime feed: {syncState === 'degraded' ? 'degraded - auto recover active' : 'healthy'}</li><li className="rounded-lg border border-zinc-800 bg-zinc-900 p-2">Booking lifecycle synchronization: aligned</li><li className="rounded-lg border border-zinc-800 bg-zinc-900 p-2">Driver telemetry cadence: stable</li></ul></Panel></div></section></div></div></div></main>;
}
