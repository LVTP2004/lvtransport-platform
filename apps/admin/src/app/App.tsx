...
type BookingLifecycle = 'pending' | 'assigned' | 'accepted' | 'en_route' | 'arrived' | 'in_progress' | 'completed' | 'cancelled' | 'failed';
type DriverOperationalState = 'offline' | 'available' | 'assigned' | 'accepted' | 'en_route' | 'arrived' | 'in_progress' | 'completed' | 'cancelled' | 'failed' | 'unknown';
...
type AdminBooking = {
  ...
  lifecycleEventLog?: Array<{ id: string; type: string; status: string; actor: string; at: string; note?: string }>;
...
const brandLogo = '/brand/lv-transport-logo.png';
...
const normalizeBookingStatus = (status: string): BookingLifecycle => {
  ...
  if (['onderweg', 'enroute', 'on_route', 'active', 'driver_arriving'].includes(s)) return 'en_route';
  if (s === 'arrived') return 'arrived';
  if (['in_progress', 'passenger_onboard'].includes(s)) return 'in_progress';
...
const normalizeDriverState = (state: string): DriverOperationalState => {
  ...
  if (['onderweg', 'enroute', 'on_route', 'driver_arriving'].includes(s)) return 'en_route';
  if (['offline', 'available', 'assigned', 'accepted', 'arrived', 'in_progress', 'completed', 'cancelled', 'failed'].includes(s)) return s as DriverOperationalState;
  return 'unknown';
};
...
export function App() {
  ...
  const bookingsWithOps = useMemo(() => bookings.map((b) => {
    ...
    const duplicateTransition = Boolean((b.lifecycleEventLog ?? []).some((event) => event.type === 'duplicate_event') || (b.timeline ?? []).some((entry, i, arr) => i > 0 && arr[i - 1]?.status === entry.status));
    const invalidTransition = Boolean((b.lifecycleEventLog ?? []).some((event) => event.type === 'invalid_transition'));
    const syncDiagnostics = (b.lifecycleEventLog ?? []).filter((event) => event.type === 'sync_diagnostic').length;
    const stale = Boolean(b.updatedAt && Date.now() - new Date(b.updatedAt).getTime() > 10 * 60_000 && !immutable);
    return { ...b, normalizedStatus, immutable, duplicateTransition, invalidTransition, syncDiagnostics, stale };
  }), [bookings]);
  ...
  const activeBookings = useMemo(() => bookingsWithOps.filter((b) => ['assigned', 'accepted', 'en_route', 'arrived', 'in_progress'].includes(b.normalizedStatus)), [bookingsWithOps]);
  ...
  return <main className="min-h-screen bg-zinc-900 text-zinc-100"><div className="grid min-h-screen grid-cols-1 lg:grid-cols-[260px_1fr]"><aside className="border-r border-zinc-800 bg-black/90 p-6"><div className="mb-8"><img src="/brand/lv-logo-primary.svg" alt="LV Transport" className="h-14 w-auto rounded-lg border border-amber-400/30 bg-black/80 p-1" /><h1 className="mt-3 text-2xl font-bold text-amber-300">Control Tower</h1></div><nav className="space-y-2">{navItems.map(({ label, icon }, index) => <button key={label} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2 text-sm ${index === 0 ? 'bg-amber-400/20 text-amber-200' : 'text-zinc-300 hover:bg-zinc-800/80 hover:text-white'}`}><span className="w-4 text-center">{icon}</span>{label}</button>)}</nav></aside><div className="flex flex-col"><header className="flex items-center justify-between border-b border-zinc-800 bg-zinc-950/80 px-5 py-4"><div className="flex items-center gap-3"><img src="/brand/lv-logo-dark.svg" alt="LV Transport" className="h-10 w-auto rounded-md border border-amber-400/20 bg-black/80 p-1" /><div><p className="text-xs uppercase tracking-[0.2em] text-zinc-400">Operations Center</p><p className="text-lg font-medium text-white">Regional Dispatch & Service Health</p></div></div><p className="text-xs text-zinc-400">Sync: <span className={syncState === 'live' ? 'text-emerald-300' : syncState === 'recovering' ? 'text-amber-200' : 'text-rose-300'}>{syncState.toUpperCase()}</span> • cycle {refreshCycle}</p></header><div className="space-y-5 p-5"><section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><MetricCard title="Realtime Bookings" value={`${bookingsWithOps.length}`} trend={`${activeBookings.length} actively supervised`} tone="gold" /><MetricCard title="Driver Operations" value={`${normalizedDrivers.length}`} trend={`${normalizedDrivers.filter((d) => d.normalizedState === 'en_route').length} en route`} tone="emerald" /><MetricCard title="Moni Escalations" value={`${unresolvedIncidents.length}`} trend="AI-human escalation visibility" tone="blue" /><MetricCard title="Operational Warnings" value={`${staleBookings.length + disconnectedDrivers.length}`} trend="stale/disconnected indicators" tone="rose" /></section>
  return <main className="min-h-screen bg-zinc-900 text-zinc-100"><div className="grid min-h-screen grid-cols-1 lg:grid-cols-[260px_1fr]"><aside className="border-r border-zinc-800 bg-black/90 p-6"><div className="mb-8"><div className="flex items-center gap-3"><img src={brandLogo} alt="LV Transport logo" className="h-11 w-11 rounded-lg border border-amber-300/30 bg-black/70 object-contain p-1" onError={(event) => { event.currentTarget.style.display = 'none'; }} /><div><p className="text-xs uppercase tracking-[0.28em] text-zinc-500">LV Transport</p><h1 className="mt-1 text-2xl font-bold text-amber-300">Control Tower</h1></div></div><p className="mt-3 text-xs text-zinc-400">Premium executive mobility monitoring · realtime operational intelligence.</p></div><nav className="space-y-2">{navItems.map(({ label, icon }, index) => <button key={label} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2 text-sm ${index === 0 ? 'bg-amber-400/20 text-amber-200' : 'text-zinc-300 hover:bg-zinc-800/80 hover:text-white'}`}><span className="w-4 text-center">{icon}</span>{label}</button>)}</nav></aside><div className="flex flex-col"><header className="flex items-center justify-between border-b border-zinc-800 bg-zinc-950/80 px-5 py-4"><div><p className="text-xs uppercase tracking-[0.2em] text-zinc-400">Operations Center</p><p className="text-lg font-medium text-white">Regional Dispatch & Service Health</p></div><p className="text-xs text-zinc-400">Sync: <span className={syncState === 'live' ? 'text-emerald-300' : syncState === 'recovering' ? 'text-amber-200' : 'text-rose-300'}>{syncState.toUpperCase()}</span> • cycle {refreshCycle}</p></header><div className="space-y-5 p-5"><section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><MetricCard title="Realtime Bookings" value={`${bookingsWithOps.length}`} trend={`${activeBookings.length} actively supervised`} tone="gold" /><MetricCard title="Driver Operations" value={`${normalizedDrivers.length}`} trend={`${normalizedDrivers.filter((d) => d.normalizedState === 'en_route').length} en route`} tone="emerald" /><MetricCard title="Moni Escalations" value={`${unresolvedIncidents.length}`} trend="AI-human escalation visibility" tone="blue" /><MetricCard title="Operational Warnings" value={`${staleBookings.length + disconnectedDrivers.length}`} trend="stale/disconnected indicators" tone="rose" /></section>
<section className="grid gap-5 xl:grid-cols-3"><div className="space-y-5 xl:col-span-2"><Panel title="Booking Lifecycle Supervision" icon={<span>◈</span>}><div className="mb-3 flex flex-wrap gap-2">{(['all', 'pending', 'assigned', 'accepted', 'en_route', 'arrived', 'in_progress', 'completed', 'cancelled', 'failed'] as const).map((filter) => <button key={filter} onClick={() => setStatusFilter(filter)} className={`rounded-lg border px-2 py-1 text-xs uppercase ${statusFilter === filter ? 'border-amber-300 text-amber-200' : 'border-zinc-700 text-zinc-300'}`}>{filter}</button>)}</div><div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-sm"><thead className="text-xs uppercase tracking-[0.16em] text-zinc-400"><tr>{['Reference', 'Service', 'Status', 'Lifecycle', 'Immutable', 'Timestamps', 'Ops Flags'].map((h) => <th key={h} className="px-2 py-2">{h}</th>)}</tr></thead><tbody>{filteredBookings.map((row) => <tr key={row.id} className="border-t border-zinc-800 text-zinc-200"><td className="px-2 py-3">{row.referenceCode ?? row.code ?? row.id}</td><td className="px-2 py-3">{row.serviceType}</td><td className="px-2 py-3">{row.status}</td><td className="px-2 py-3">{row.normalizedStatus}</td><td className="px-2 py-3">{row.immutable ? 'locked' : 'mutable'}</td><td className="px-2 py-3 text-xs">{row.createdAt ? new Date(row.createdAt).toLocaleTimeString() : '-'} / {row.updatedAt ? new Date(row.updatedAt).toLocaleTimeString() : '-'}</td><td className="px-2 py-3 text-xs">{row.duplicateTransition ? 'duplicate visible' : 'clean'} {row.invalidTransition ? '• invalid seen' : ''} {row.syncDiagnostics ? `• sync:${row.syncDiagnostics}` : ''} {row.stale ? '• stale' : ''}</td></tr>)}</tbody></table></div></Panel>
