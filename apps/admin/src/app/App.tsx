...
type AdminBooking = { id: string; referenceCode: string; serviceType: string; status: string; scheduledAt: string; assignedDriverName?: string; version?: number };
...
type IncidentItem = { id: string; title: string; severity: 'info'|'low'|'medium'|'high'|'critical'; category: string; state: 'open'|'acknowledged'|'resolved'; owner: string; createdAt: string; escalationLevel: number; recoveryDeadlineAt?: string; bookingId?: string; timeline: Array<{ at: string; actor: string; action: string; note?: string }> };
...
export function App() {
  ...
  const assignDriver = async (booking: AdminBooking) => {
    if (booking.assignedDriverName || booking.status !== 'pending') return;
