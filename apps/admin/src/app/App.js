...
const normalizeBookingStatus = (status) => {
    ...
    if (s === 'accepted')
        return 'accepted';
    if (['onderweg', 'enroute', 'on_route', 'active', 'driver_arriving'].includes(s))
        return 'en_route';
    if (s === 'arrived')
        return 'arrived';
    if (['in_progress', 'passenger_onboard'].includes(s))
        return 'in_progress';
...
const normalizeDriverState = (state) => {
    ...
    if (['onderweg', 'enroute', 'on_route', 'driver_arriving'].includes(s))
        return 'en_route';
    if (['offline', 'available', 'assigned', 'accepted', 'arrived', 'in_progress', 'completed', 'cancelled', 'failed'].includes(s))
        return s;
    return 'unknown';
};
export function App() {
